...
| 问题 | 触发条件 | 当前行为 | 目标行为 |
|------|----------|----------|----------|
| Payload 超限 | 序列化后 > ~615KB | 收到误导性 400 "Improperly formed request."，直接报错 | 发送前裁剪旧历史，修复孤立 toolResults |
...
Kiro API 在 payload 超过 ~615KB 时返回误导性错误 `"Improperly formed request."（reason: null）`，无法与普通参数错误区分。需要在发送前检查并自动裁剪。
...
export function guardPayload(payload) {
    ...
    if (AUTO_TRIM_PAYLOAD) {
        ...
        if (stats.finalBytes > MAX_PAYLOAD_BYTES) {
            return `Payload size (${stats.finalBytes} bytes) exceeds Kiro API limit (${MAX_PAYLOAD_BYTES} bytes) even after trimming. Try reducing message history or tools.`;
        }
        return null;
    }

    return `Payload size (${size} bytes) exceeds Kiro API limit (${MAX_PAYLOAD_BYTES} bytes). Set KIRO_AUTO_TRIM_PAYLOAD=true to auto-trim.`;
}
```
...
const KIRO_CONTEXT_LIMITS = {
    ...
    MAX_PAYLOAD_BYTES: 600000,          // 600KB，低于 Kiro 硬限 ~615KB
